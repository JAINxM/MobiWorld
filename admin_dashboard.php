...
<a href="<?php echo htmlspecialchars(adminLink('edit-product', $filter), ENT_QUOTES, 'UTF-8'); ?>" class="sidebar-link <?php echo $section === 'edit-product' ? 'active' : 'text-slate-500 hover:bg-slate-50'; ?> flex items-center rounded-2xl px-5 py-4 font-bold transition"><i class="fas fa-pen-to-square mr-4"></i> Edit Product</a>
    ...
    <main class="flex-1 p-4 md:p-8 xl:p-10">
        ...
        <section class="mb-10 grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-4">
            <div class="panel-card rounded-[2rem] border border-slate-100 p-8 shadow-sm"><div class="mb-6 flex h-14 w-14 items-center justify-center rounded-2xl bg-blue-50 text-blue-600"><i class="fas fa-indian-rupee-sign text-xl"></i></div><div class="text-4xl font-extrabold text-slate-800">₹<?php echo number_format($section === 'dashboard' ? $filteredRevenue : $totalRevenue, 2); ?></div><p class="mt-3 text-xs font-bold uppercase tracking-[0.25em] text-slate-400"><?php echo $section === 'dashboard' ? $rangeLabel . ' Revenue' : 'Total Revenue'; ?></p></div>
